...
class DynamicItem extends Component {
  ...
  handleGoLine() {
    this.props.navigation.navigate('RelationChain', {
      uid: get(this.state, 'feed.userVO.userId', ''),
    })
  }
  ...
  render() {
    ...
    const images = picList.map((dataItem, position) => {
      return { uri: dataItem }
    })
    return (
      <View style={styles.dynamicItemWrap}>
        <View style={styles.itemHeader}>
          <TouchableOpacity onPress={() => this.handlegoStrangerPage()}>
            <Image
              ...
              source={{
                uri: get(feed.userVO, 'pic', defaultAvatar).replace(
                  'https',
                  'http'
                ),
              }}
            ...
            <Text style={styles.name}>
              {
                ...
                // this.state.feed.userVO.userName
              }
            </Text>
            <View style={styles.infoBox}>
              <Text style={styles.infoPosition}>{feed.cityName || '北京'}</Text>
              <View style={styles.line} />
              <Text style={styles.infoTime}>{getDate(feed.publishTime)}</Text>
            </View>
          </View>
        ...
        <View style={styles.dynamicTextBox}>
          <TouchableOpacity onPress={() => this.handleGoDetail()}>
            <Text style={styles.dynamicText}>{feed.content}</Text>
          </TouchableOpacity>
