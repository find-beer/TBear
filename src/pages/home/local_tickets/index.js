...
class LocalTickets extends React.Component {
  constructor(props) {
    super(props)
    console.log('LocalTickets==================', props)
    this.state = {
      ticketVoList: props.route.params.ticketVoList
        ? props.route.params.ticketVoList
        : [],
    }
  }
  ...
  deleteTicket = async (index) => {
    const { ticketVoList } = this.state
    ticketVoList.splice(
      ticketVoList.findIndex((item, position) => index === position),
      1
    )
    this.setState({
      ticketVoList: ticketVoList.splice(
        ticketVoList.findIndex((item, position) => index === position),
        1
      ),
    })
      ...
      <View>
        ...
        >
          <View style={styles.tickeCardContent}>
            <View style={styles.tickeTypeSituation}>
              <Text style={styles.tickeType}>票种</Text>
              <Text style={styles.studentTicket}>{data.ticketName}</Text>
            </View>
            <View style={styles.tickeTypeSituation}>
              <Text style={styles.GroupPrice}>价格</Text>
              <Text style={styles.tickePrice}>{data.price + ''}元</Text>
            </View>
          </View>
        ...
        <View style={{ marginLeft: scaleSize(54) }}>
          ...
          <Text style={{ color: '#999', marginRight: scaleSize(54) }}>
            {data.illustration}
          </Text>
        </View>
      </View>
